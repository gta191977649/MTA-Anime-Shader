...
//-- PixelShader
//------------------------------------------------------------------------------------------
Pixel PixelShaderStratos(PSInput PS)
{
    Pixel output;
    ...
    if ((1 - PS.NightFade.x)>0) 
    {
        ...
        nightSky = lerp(float4((fogImpact.rgb + nightSky.rgb), nightSky.a), nightSky, fadeDot);
        fadeDot = lerp(fadeDot, 1, saturate(1 - PS.NightFade.x));
    }
    ...
    output.World = saturate(nightSky);
    output.Depth = 0.99999f;
    return output;
}

Pixel PixelShaderDots(PSInput PS)
{
    Pixel output;
