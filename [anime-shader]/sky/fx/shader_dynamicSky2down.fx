...
struct PSInput
{
    ...
    float3 NightFade : TEXCOORD1;
    float3 PositionVS : TEXCOORD2;
    float3 LimitDot : TEXCOORD3;
...
PSInput VertexShaderSB(VSInput VS)
{
    ...
    float3 eyeVector = normalize(worldPos.xyz - 0); 	 
    ...
    if (gHorizonBlending) PS.NightFade.y = saturate(MTAUnlerp(farClip + abs(gBottCloudSpread) + 50, farClip + 50, gCameraPosition.z));
        else PS.NightFade.y = 0.5 * PS.NightFade.y;
    PS.NightFade.z = saturate(MTAUnlerp(gStratosFade[0], gStratosFade[1], gCameraPosition.z));

    if (gHorizonBlending) PS.LimitDot.x = saturate(2.2 * PS.PositionVS.z);
        else PS.LimitDot.x = 1.0;
    if (gHorizonBlending) PS.LimitDot.z = saturate(-2.2 * PS.PositionVS.z);
        else PS.LimitDot.z = 1.0;
...
//-- PixelShader
//------------------------------------------------------------------------------------------
Pixel PixelShaderClouds(PSInput PS)
{
    Pixel output;
    ...
    outPut = saturate(outPut * pow(PS.NightFade.x,5));
    ...
    if ((1 - PS.NightFade.x)>0) 
    {
        ...
        nightSky.rgba *= saturate(1 - PS.NightFade.z);
		
        nightSky.a = PS.LimitDot.z * saturate(1 - PS.NightFade.x) * pow(PS.LimitDot.y, 0.3);
        float3 fogImpact = saturate(1 - nightSky.rgb) * gFogColor.rgb;
        float fadeMul = saturate(1 - PS.NightFade.x) * saturate(1 - PS.NightFade.z) * fadeDot;
        outPut = lerp(outPut, float4((fogImpact.rgb + nightSky.rgb), nightSky.a), fadeMul);
    }

    output.World = saturate(outPut);
