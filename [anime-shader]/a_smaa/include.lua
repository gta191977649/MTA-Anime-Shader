...
-- Это дерьмо нужно адекватно переписать (но т.к. классы не мои и чисто shared для публики можно и пренебречь. в проприетарном всё хорошо)
function bindEvent(name, attached, callback, _self)
    addEventHandler(name, attached, function(...)
        callback(_self, ...)
    end)
end