...
texture SMAAEdgeMap < string renderTarget = "yes"; >;
...
texture SMAABlendMap < string renderTarget = "yes"; >;
...
texture SMAANeighborHoodMap < string renderTarget = "yes"; >;
sampler SMAANeighborHoodSamp = sampler_state {
	texture = <SMAANeighborHoodMap>;
	MinFilter = POINT; MagFilter = POINT; MipFilter = NONE;
	AddressU  = CLAMP; AddressV = CLAMP; MaxMipLevel = 0; MipMapLodBias = 0;
};
...
/* SMAA Edge Detection. < RENDERTARGET > */
...
struct PixelEdgeDetection
{
	float4 World : COLOR0; // < RENDERTARGET #0 >
	float4 EdgeColor : COLOR1; // < RENDERTARGET #1 >
	float4 BlendWeight : COLOR2; // < RENDERTARGET #2 >
	float4 NeighborHood : COLOR3; // < RENDERTARGET #3 >
};

PixelEdgeDetection SMAALumaEdgeDetectionPS(OutputSMAAEdgeDetectionVS input) // PASS 0
{
	PixelEdgeDetection output;
	...
	float4 Edge = float4(edges * step(maxDelta * 0.5, delta.xy), 0.0, 0.0); // DEBUG
	
	output.World = tex2D(ScnSamp, input.TexCoord);//(float4)1;
	output.EdgeColor = Edge; // PASS 0
	output.BlendWeight = (float4)0; // PASS 1
	output.NeighborHood = (float4)0; // PASS 2
...
PixelBlendingWeight SMAABlendingWeightCalculationPS(OutputSMAABWCalculationVS input) // PASS 1
{
	PixelBlendingWeight output;
	...
	output.World = tex2D(ScnSamp, input.TexCoord);//(float4)1;
	output.EdgeColor = tex2D(SMAAEdgeMapSamp, input.TexCoord); // PASS 0
	output.BlendWeight = weights; // PASS 1
	output.NeighborHood = 0; // PASS 2
...
PixelNeighood SMAANeighborhoodBlendingPS(OutputSMAANWCalculationVS input) // PASS 2
{
	PixelNeighood output = (PixelNeighood)0;
	output.World = tex2D(ScnSamp, input.TexCoord);//(float4)1;
	output.EdgeColor = tex2D(SMAAEdgeMapSamp, input.TexCoord); // PASS 0
	output.BlendWeight = tex2D(SMAABlendMapSamp, input.TexCoord); // PASS 1
	output.NeighborHood = 0; // PASS 2
	...
	if (dot(a, 1) < 1e-5) 
	{
		...
		output.NeighborHood = float4(color.rgb, 1);
		return output;
	}
	else 
	{
